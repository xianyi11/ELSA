...
class LsqQuanAct(t.nn.Module):
    def __init__(self, bit, all_positive=False, symmetric=False, per_channel=True):
        ...
        if all_positive:
            ...
            if symmetric:
                ...
                # signed weight/activation is quantized to [-2^(b-1), 2^(b-1)-1] (ResNet50: upper bound +2, see glo)
                self.thd_neg = - 2 ** (bit - 1)
                if glo.get_value("resnet50_lsq_extend_thd_pos", False):
                    self.thd_pos = 2 ** (bit - 1) + 1
                else:
                    self.thd_pos = 2 ** (bit - 1) - 1
    ...
    def __repr__(self):
        return f"LsqQuanAct(thd_pos={self.thd_pos}, thd_neg={self.thd_neg}, s={self.s.data}, per_channel={self.per_channel})"
...
class LsqQuan(t.nn.Module):
    ...
    def init_from(self, x, weight=True, *args, **kwargs):
        if weight == True:
            threshold = threshold_optimization(np.array(x.detach().cpu()), quantization_level=int(self.thd_pos+1), n_trial=300, eps=1e-10)
            self.s.data = torch.tensor(threshold / (self.thd_pos),dtype=torch.float32).cuda()
            # if self.per_channel:
            #     self.s = t.nn.Parameter(
            #         x.detach().abs().mean(dim=list(range(1, x.dim())), keepdim=True) * 2 / (self.thd_pos ** 0.5))
...
class QuanInferAvgPool(t.nn.Module):
    def __init__(self,m:QuanAvgPool, last_quan_out_fn, name, kernel_size = None):
        super(QuanInferAvgPool,self).__init__()
        self.m = m
        self.last_quan_out_fn = last_quan_out_fn
        if hasattr(self.m.m,"kernel_size"):
...
class QuanConv2dFuseBN(t.nn.Conv2d):
    def __init__(self, m: t.nn.Conv2d, quan_w_fn=None, quan_a_fn=None, quan_out_fn=None,momentum=0.9, eps=1e-5,is_first=False):
        ...
        self.eps = eps
        self.register_parameter('beta', t.nn.Parameter(torch.zeros(m.out_channels), requires_grad=False))
        self.register_parameter('gamma', t.nn.Parameter(torch.ones(m.out_channels), requires_grad=False))
    ...
    def forward(self, x):
        ...
        #     print("Training input int", (x).abs().mean())
        # if self.is_init == False:
        #     self.quan_w_fn.init_from(weight,weight=True)
        ...
        # print("QAT output",(quantized_out/self.quan_out_fn.s).abs().mean()zed_bias.reshape(1,-1,1,1),min=self.quan_out_fn.s*self.quan_out_fn.thd_neg,max=self.quan_out_fn.s*self.quan_out_fn.thd_pos)
        # print("QAT output",(quantized_out/self.quan_out_fn.s).abs().mean())
...
class QuanInferConv2dFuseBN(t.nn.Conv2d):
    ...
    def forward(self,x):
        ...
        # print("infer output:",torch.abs(x).mean())
...
class QuanLinear(t.nn.Linear):
    ...
    def forward(self, x):        
        ...
        out = t.nn.functional.linear(x, quantized_weight, bias=self.bias)
...
class QuanInferLinear(t.nn.Linear):
    def __init__(self, m: QuanLinear, last_quan_out_fn, is_first=False, name="act"):
        ...
        self.weight = t.nn.Parameter(m.quan_w_fn(m.weight.detach())/m.quan_w_fn.s,requires_grad=False)
        if m.bias is not None:
            self.bias = t.nn.Parameter(m.bias.detach(),requires_grad=False)
        s = last_quan_out_fn.s*m.quan_w_fn.s
        N,M = get_M_N(s)
    ...
    def forward(self,x):
        ...
        out = (float(self.M)/float(2**self.N))*t.nn.functional.linear(x, self.weight) + self.bias
...
def quantized_inference_model_fusebn(model):
    ...
    for name, child in children:
        ...
        if isinstance(child, BasicBlock) or isinstance(child, BasicBlockCifar) or isinstance(child, BasicBlockResNet12):
            ...
            index = index + 1
            if len(child.conv) == 5:
                model._modules[name].ResidualAdd = AdditionInfer(m=child.ResidualAdd,quan_input1_fn=child.conv[3].m.quan_out_fn,quan_input2_fn=last_quan_out_fn)
                if child.identity == False:
                    last_quan_out_fn = child.conv[3].m.quan_out_fn
                else:
                    last_quan_out_fn = child.ResidualAdd.m.quan_a_fn
            elif len(child.conv) == 8:
                ...
                index = index + 1
                model._modules[name].ResidualAdd = AdditionInfer(m=child.ResidualAdd,quan_input1_fn=child.conv[6].m.quan_out_fn,quan_input2_fn=last_quan_out_fn)
                if child.identity == False:
                    last_quan_out_fn = child.conv[6].m.quan_out_fn
                else:
                    last_quan_out_fn = child.ResidualAdd.m.quan_a_fn
            ...
            model._modules[name] = QuanInferAvgPool(child, last_quan_out_fn=last_quan_out_fn, name=f"AvgPooling{index}",kernel_size=7)
            index = index + 1
            is_need = True
            last_quan_out_fn = child.quan_out_fn
        if not is_need:
    ...
    for name, child in children:
        ...
        if isinstance(child, BasicBlock) or isinstance(child, BasicBlockCifar) or isinstance(child, BasicBlockResNet12):
            ...
            model._modules[name].conv[3] = QuanConv2dFuseBN(m=child.conv[3], is_first=(index2==0), quan_w_fn=LsqQuan(bit=weightBit,all_positive=False,symmetric=False,per_channel=False),
                                            quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False),
                                            quan_out_fn=LsqQuanAct(bit=actBit,all_positive=False if len(child.conv) == 8 else False,symmetric=False,per_channel=False))
            index2 = index2 + 1
            if len(child.conv) == 8:
            ...
            model._modules[name].conv1 = QuanConv2dFuseBN(m=child.conv1, is_first=(index2==0), quan_w_fn=LsqQuan(bit=weightBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_out_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False))
            # model._modules[name].conv1.quan_out_fn.s.data = torch.tensor(BottleneckIndex/3)
            index2 = index2 + 1
            model._modules[name].conv2 = QuanConv2dFuseBN(m=child.conv2, is_first=(index2==0), quan_w_fn=LsqQuan(bit=weightBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_out_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False))
            # model._modules[name].conv2.quan_out_fn.s.data = torch.tensor(BottleneckIndex/3)
            index2 = index2 + 1
            model._modules[name].conv3 = QuanConv2dFuseBN(m=child.conv3, is_first=(index2==0), quan_w_fn=LsqQuan(bit=weightBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_out_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False))
            # model._modules[name].conv3.quan_out_fn.s.data = torch.tensor(BottleneckIndex/3)
            index2 = index2 + 1
            if child.downsample is not None:
                child.downsample[0] = QuanConv2dFuseBN(m=child.downsample[0], is_first=(index2==0), quan_w_fn=LsqQuan(bit=weightBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False),
                                                quan_out_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False))
                # child.downsample[0].quan_out_fn.s.data = torch.tensor(BottleneckIndex/3)
                index2 = index2 + 1
            # else:
            #     child.downsample = model._modules[name].conv3.quan_out_fn
            child.ResidualAdd = AdditionQuan(quan_a_fn=LsqQuanAct(bit=actBit,all_positive=False,symmetric=False,per_channel=False))            
            # child.ResidualAdd.quan_a_fn.s.data = torch.tensor(BottleneckIndex/3)
            is_need = True
        elif isinstance(child, t.nn.Linear):
            _qm = glo.get_value("quan_target_model", "")
            if _qm == "resnet18":
                _fc_w, _fc_a = 8, 8
            else:
                _fc_w, _fc_a = weightBit, actBit
            model._modules[name] = QuanLinear(m=child, quan_w_fn=LsqQuan(bit=_fc_w,all_positive=False,symmetric=False,per_channel=False),
                                            quan_out_fn=LsqQuanAct(bit=_fc_a,all_positive=False,symmetric=False,per_channel=False))
            index2 = index2 + 1
            is_need = True
        elif isinstance(child, t.nn.Conv2d):
            if index2 == 0:
            ...
            # model._modules[name].quan_out_fn.s.data = torch.tensor(BottleneckIndex/3)
            index2 = index2 + 1
            is_need = True
        if not is_need:
            quantized_train_model_fusebn(child,weightBit, actBit)
