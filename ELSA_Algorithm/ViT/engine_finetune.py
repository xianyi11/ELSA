...
from spike_quan_layer import l2_regularization
...
def train_one_epoch_distill(model: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            outputs_teacher = model_teacher(samples)
            loss = criterion(outputs, targets)
            loss_distill = get_logits_loss(outputs_teacher, outputs, targets_one_hot, args.temp)
            if args.prune:
                l2_loss, abs_value = l2_regularization(model)
                l2_loss = l2_loss*args.choose_prune/args.prune_ratio
                abs_value = abs_value/args.prune_ratio
            loss_all = loss + loss_distill + (l2_loss if args.prune else 0.0)
        loss_value = loss.item()
        loss_distill_value = loss_distill.item()
        if args.prune:
            abs_value = abs_value.item()
        ...
        if args.prune:
            metric_logger.update(loss_all=loss_all_value, loss=loss_value, loss_distill=loss_distill_value, l2_loss = l2_loss_value, abs_value=abs_value)
        else:
            metric_logger.update(loss_all=loss_all_value, loss=loss_value, loss_distill=loss_distill_value)
        min_lr = 10.
        max_lr = 0.
        ...
        loss_distill_value_reduce = misc.all_reduce_mean(loss_distill_value)
        if args.prune:
            l2_loss_value_reduce = misc.all_reduce_mean(l2_loss_value)
            abs_value_reduce = misc.all_reduce_mean(abs_value)
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            """ We use epoch_1000x as the x-axis in tensorboard.
            This calibrates different curves when batch size changes.
            """
            ...
            log_writer.add_scalar('loss_distill', loss_distill_value_reduce, epoch_1000x)
            if args.prune:
                log_writer.add_scalar('loss_l2', l2_loss_value_reduce, epoch_1000x)
                log_writer.add_scalar('abs_value', abs_value_reduce, epoch_1000x)
            log_writer.add_scalar('lr', max_lr, epoch_1000x)
            if args.wandb:
                wandb.log({'loss_all_curve': loss_all_value_reduce}, step=epoch_1000x)
                wandb.log({'loss_curve': loss_value_reduce}, step=epoch_1000x)
                wandb.log({'loss_distill_curve': loss_distill_value_reduce}, step=epoch_1000x)
                if args.prune:
                    wandb.log({'loss_l2_curve': l2_loss_value_reduce}, step=epoch_1000x)
                    wandb.log({'abs_value_curve': abs_value_reduce}, step=epoch_1000x)
                wandb.log({'lr_curve': max_lr}, step=epoch_1000x)
...
@torch.no_grad()
def evaluate(data_loader, model, device, mode, args):
    criterion = torch.nn.CrossEntropyLoss()
