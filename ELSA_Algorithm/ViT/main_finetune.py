...
from spike_quan_wrapper_ICML import myquan_replace, SNNWrapper
...
def get_args_parser():
    ...
    parser.add_argument('--confidence_thr', default=0.3, type=float, help='confidence threshold for early exit')
    return parser
...
def main(args):
    ...
    if args.mode.count("QANN") > 0:
        ...
        # trunc_normal_(model.head.weight, std=2e-5)
        if args.elastic:
            model = SNNWrapper(ann_model=model, cfg=None, time_step=args.time_step, \
                           Encoding_type=args.encoding_type, level=args.level, neuron_type=args.neuron_type, \
                           model_name=args.model, is_softmax = not args.remove_softmax, \
                           record_inout=args.record_inout, record_dir=args.log_dir+f"/output_bin_snn_{args.model}_w{args.weight_quantization_bit}_a{int(torch.log2(torch.tensor(args.level)))}_T{args.time_step}/", confidence_thr=args.confidence_thr)
        else:
            model = SNNWrapper(ann_model=model, cfg=None, time_step=args.time_step, \
                            Encoding_type=args.encoding_type, level=args.level, neuron_type=args.neuron_type, \
                            model_name=args.model, is_softmax = not args.remove_softmax, \
                            record_inout=args.record_inout, record_dir=args.log_dir+f"/output_bin_snn_{args.model}_w{args.weight_quantization_bit}_a{int(torch.log2(torch.tensor(args.level)))}_T{args.time_step}/")
    ...
    if args.distributed:
        ...
        model_qann = torch.nn.parallel.DistributedDataParallel(model_qann, device_ids=[args.gpu])
