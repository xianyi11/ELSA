...
class GroupScaling1D(nn.Module):
    ...
    def forward(self, input):
        ...
        # moment2 = torch.repeat_interleave(torch.repeat_interleave( torch.mean(gn_input * gn_input, dim=[0,3], keepdim=True), \
        #     repeats=Cg, dim=-1), repeats=T, dim=0).contiguous().reshape(T, B, C)
        # divide out second moment
        # print(torch.mean(gn_input * gn_input, dim=[0,3], keepdim=True).reshape(-1))
...
class MaskPowerNorm(nn.Module):
    ...
    def forward(self, input, pad_mask=None, is_encoder=False):
        """
        input:  B x T x C -> T x B x C -> B x C x T
             :  B x C x T -> T x B x C
        ...
        T, B, C = input.shape
        print("mean",input.mean(dim=-1)[0].reshape(-1).min(),input.mean(dim=-1)[0].reshape(-1).max())
        print("std",input.std(dim=-1).reshape(-1).min(),input.std(dim=-1).reshape(-1).max())
