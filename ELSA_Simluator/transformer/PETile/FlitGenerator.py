...
class FlitGenerator(VSAModule):
    def __init__(self,RouteInfo, PEId, TileValue, N=1):
    ...
    def forward(self,spikes:torch.tensor, columnId:int, headId:int, ColFirst:bool=True):
        ...
        if not ColFirst:
            rowId = columnId
        N = spikes.shape[0]
        # print("spikes",spikes,"ColFirst",ColFirst)
        # print("headId",headId)
        while(1):    
            if curSpikePoint >= N:
                break
            ...
            if spikes[curSpikePoint] != 0:
                ...
                if flitNum == 0:
                    ...
                    if ColFirst:
                        payloadColumn.append(columnId+self.N*headId)
                    else:
                        payloadRow.append(rowId+self.N*headId)
                    flitType = "01"
                ...
                while(1):
                    ...
                    if spikes[curSpikePoint] != 0:
                        ...
                        if ColFirst:
                            ...
                            payloadColumn.append(curSpikePoint+self.PEId*self.TileValue)
        ...
        if len(flitList) != 0:
            ...
            thePayload = payLoad()
            if ColFirst:
                payloadColumn = [columnId+self.N*headId]
                payloadRow = []
            else:
                payloadColumn = []
                payloadRow = [rowId+self.N*headId]
            sign = []
            flitType = "01"
            flitRouteInfo = self.RouteInfo
            thePayload.setup(columnId=payloadColumn,rowId=payloadRow,sign=sign)
...
class FlitCombiner(VSAModule):
    ...
    def forward(self,flitList:List[Flit]):
        if len(flitList) == 0:
            ...
            self.colFirst = flitList[0].colFirst
            if self.colFirst == True:
                if self.columnID == -1:
                    self.columnID = flitList[0].Payload.columnId[0]
            else:
                if self.rowID == -1:
                    self.rowID = flitList[0].Payload.rowId[0]
            for i,flit in enumerate(flitList):
                # print(flit.printmyself())
                self.colFirst = flit.colFirst
                if self.colFirst:
                    if i == 0:
                    ...
                    if i == 0:
                        rowID = flit.Payload.rowId[0]
                        assert self.rowID == rowID, f"From FlitCombiner!!!! the rowID mush be same. self.rowID:{self.rowID} != rowID:{rowID}"
                    for columnId,sign in zip(flit.Payload.columnId, flit.Payload.sign):
                        self.spikes.append((rowID,columnId,sign))
                # print("in flitcombiner:",flit.time)
                self.maxcycle = max(self.maxcycle, flit.time + len(flitList))
                if flit.tail:
                    self.tailNum = self.tailNum + 1
        if self.tailNum == self.PENums:
            if self.colFirst is None: #no spike through
                self.tailNum = 0
                self.spikes = []
                self.maxcycle = 0
                return None
            ...
            if self.colFirst:
                ...
                self.maxcycle = 0
                self.columnID = -1
                return spikes, columnID, maxcycle, self.colFirst
            else:
                rowID = self.rowID + 0
                self.maxcycle = 0
                self.rowID = -1
                return spikes, rowID, maxcycle, self.colFirst
        else:
            return None
