...
class membrane(VSAModule):
    ...
    def __init__(self, N, M, **kwargs):
        ...
        self.SRAMNumber = N//self.busWidth if N % self.busWidth == 0 else N//self.busWidth + 1
        self.tileHN = self.busWidth
        self.tileWN = 1
    ...
    def output_data(self, block_id, row_id):
        return self.srams[block_id].read(row_id)[:self.M]

    def fake_output_data(self, block_id, row_id):
        return self.srams[block_id].fake_read(row_id)[:self.M]

class membraneCRAM(VSAModule):
    # careful!!!! The M is tiled by PE number P
    def __init__(self, N, M, **kwargs):
        super(membraneCRAM,self).__init__()
        self.N = N
        self.M = M
        self.srams = []
        self.busWidth = cfg["processElement"]["membrane"]["height"]
        ...
        self.CRAMNumberWidth = int(math.ceil(M/self.inoutWidth))
        
        self.cram = CRAM(busWidth=self.busWidth,inoutWidth=self.inoutWidth,matrixWidth=M, matrixHeight=N, mux=self.mux,temporature=self.temporature, voltage=self.voltage, periphery_vt=self.periphery_vt, belong="weight")
    ...
    def fake_output_data(self, rowColId, direction):
        return self.cram.fake_read(rowColId,direction=direction)
        
    def output_data(self, rowColId, direction):
        return self.cram.read(rowColId,direction=direction)
