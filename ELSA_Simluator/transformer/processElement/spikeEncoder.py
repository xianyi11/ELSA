...
class spikeEncoderRow(VSAModule):
    def __init__(self):
        super(spikeEncoderRow,self).__init__()
        self.fEnergy = cfg["processElement"]["spikeEncoder"]["fEnergy"]
        self.fCount = 0 

    def forward(self,spikes,row_id,outspikes):
        
        for i,spike in enumerate(spikes):
            if spike == 0:
                continue
            elif spike > 0:
                outspikes.append((row_id, i, 0))
                self.fCount = self.fCount + 1
            elif spike < 0:
                outspikes.append((row_id, i, 1))
                self.fCount = self.fCount + 1
        return outspikes
