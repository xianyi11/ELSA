...
class AdderTree(VSAModule):
    ...
    def forward(self, memPotential, weightData):
        ...
        for i, weight in enumerate(weightData):
...
class Adder(VSAModule):
    ...
    def forward(self, memPotential, weightData):
        ...
        for weight in weightData:
            ...
            if weight.shape[0]/self.parallelism < 1:
                self.fCount = self.fCount + weight.shape[0]/self.parallelism
            else:
                self.fCount = self.fCount + math.ceil(weight.shape[0]/self.parallelism)
        return memPotential
