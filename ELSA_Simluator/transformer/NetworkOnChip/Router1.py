...
class NOC(VSAModule):
    def __init__(self, connection_path="", mapping_path="", tranOccupy_path=""):
        ...
        self.flitNumber = 0
        print("self.tranOccupy",self.tranOccupy)
    ...
    def forward(self, curLayerId, spineTokenId, timestep, flitNumber): # 通过NoC确定token真正的输入时间，考虑数据依赖以及NoC上的传输时间
        ...
        # if curLayerId.count("addition") > 0: 
        ...
        for lastLayerId in lastLayersId: # 查询每一层网络层
            ...
            if SNode.Id == TNode.Id: 
                ...
                # if curLayerId.count("addition") > 0: 
                #     print("lastLayersId",lastLayersId,"self.tokenSpineOutputTime[lastLayerId][timestep][spineTokenId]",self.tokenSpineOutputTime[lastLayerId][timestep][spineTokenId],"self.RouterCycle",self.RouterCycle,"SNode.occupy",SNode.occupy,"flitNumber",flitNumber,"curSpineInputTime",curSpineInputTime)
                    ...
                    while(1): # 一直走直到终点
                        if x_now == x2 and y_now == y2: # 到达终点
                            ...
                            if x_now == x1 and y_now == y1:
                                y_prod = SNode.YAllocate
                                x_prod = SNode.XAllocate
                                # 归一化
                                if y_prod + x_prod > 0:
                                    y_prod = y_prod/(y_prod + x_prod)
                                    x_prod = x_prod/(y_prod + x_prod)
                                else:
                                    y_prod = 0.5
                                    x_prod = 0.5
                            else:
