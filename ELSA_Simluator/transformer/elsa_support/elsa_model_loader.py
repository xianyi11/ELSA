...
from elsa_support.paths import TRANSFORMER_ROOT


def load_elsa_models_config(path: str | Path | None = None) -> Dict[str, Any]:
    p = Path(path) if path else TRANSFORMER_ROOT / "configs" / "elsa_models.yaml"
...
def resolve_data_paths(
    entry: Dict[str, Any], transformer_root: Path | None = None
) -> Dict[str, str]:
    root = transformer_root or TRANSFORMER_ROOT
