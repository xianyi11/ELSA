...
class VSAUpdateArbiter(VSAModule):
    ...
    def forward(self,i,j):
        ...
        if self.KH == 1 and self.KW == 1 and self.IH == 1 and self.IW == 1:
            return True, [0], [0]
        ...
        # 考虑kernel_size=1的情况
        update = (i == j and self.KW==self.KH and self.KW==1) or (i!=j and (curMaxIndex - i + 1 >= self.KH and i<j or j+1 >= self.KW and i>j)) 
