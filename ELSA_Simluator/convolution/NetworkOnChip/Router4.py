...
class NOC(VSAModule):
    def __init__(self, connection_path="", mapping_path="", tranOccupy_path=""):
        ...
        self.NoCCycle1 = self.flitSize1/self.NoCDatawidth
        self.NoCCycle2 = self.flitSize2/self.NoCDatawidth
        self.NoCCycle3 = self.flitSize3/self.NoCDatawidth
    ...
    def forward(self, curLayerId, spineTokenId, timestep, flitNumber): # 通过NoC确定token真正的输入时间，考虑数据依赖以及NoC上的传输时间
        ...
        for lastLayerId in lastLayersId: # 查询每一层网络层
            ...
            if SNode.Id == TNode.Id: 
                ...
                self.transmitTraffic[pos][pos] = self.transmitTraffic[pos][pos] + flitNumber * self.flitSize3
                        ...
                        while(1): # 一直走直到终点
                            if x_now == x2 and y_now == y2: # 到达终点
                                ...
                                if x_now - 1 >= 0: 
                                    ...
                                    x_allocate_re_dx = 0
                                # 归一化
                                y_allocate_tmp = y_allocate/(y_allocate + x_allocate_dx + x_allocate_re_dx)
                                x_allocate_dx_tmp = x_allocate_dx/(y_allocate + x_allocate_dx + x_allocate_re_dx)
                                x_allocate_re_dx_tmp = x_allocate_re_dx/(y_allocate + x_allocate_dx + x_allocate_re_dx)
                                ...
                                if flitId < x_allocate_dx * flitNumber: # 往x轴正方向走
                                    ...
                                    curNocLinkTranTime = curNocLinkTranTime + math.ceil(self.tranOccupy[pos1][pos2] * self.NoCCycle1) + self.RouterCycle * curNode.occupy
                                    self.transmitTraffic[pos1][pos2] = self.transmitTraffic[pos1][pos2] + self.flitSize1
                                    ...
                                    # NoC link time + Router time
                                    curNocLinkTranTime = curNocLinkTranTime + math.ceil(self.tranOccupy[pos1][pos2] * self.NoCCycle2) + self.RouterCycle * curNode.occupy
                                    self.transmitTraffic[pos1][pos2] = self.transmitTraffic[pos1][pos2] + self.flitSize2
                    ...
                    for flitId in range(flitNumber): # 对于每一个输入的flit
                        ...
                        while(1): # 一直走直到终点
                            if x_now == x2 and y_now == y2: # 到达终点
                                ...
                                # 归一化
                                y_allocate_dy_tmp = y_allocate_dy/(y_allocate_dy + y_allocate_re_dy + x_allocate)
                                y_allocate_re_dy_tmp = y_allocate_re_dy/(y_allocate_dy + y_allocate_re_dy + x_allocate)
                                x_allocate_tmp = x_allocate/(y_allocate_dy + y_allocate_re_dy + x_allocate)
