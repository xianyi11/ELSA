...
import math
import design_exploration_multiPath_optimization.partition as partition
import random
import sys
sys.path.append("./design_exploration_multiPath_optimization")
...
class NOC(VSAModule):
    def __init__(self, connection_path="", mapping_path="", tranOccupy_path=""):
        ...
        # 每个 router 的“忙到何时”：下一 spine 的 flit 到达时若早于该时间则需等待（考虑占用）
        self.router_busy_until = [0.0] * self.nodeNum
    
    def reset_router_busy_until(self):
        """每层开始前由编译器调用，清空各 router 的占用状态，以便本层内按 spine 顺序累积等待。
        调用时机：在该层 for t in range(T) / for column 循环之前调用一次。"""
        self.inputFlitNum = 0
        self.router_busy_until = [0.0] * self.nodeNum

    def _router_wait_and_forward(self, arrival_time, pos, node):
        """计算在 pos 处 router 的等待与转发：若当前仍被上一 spine 占用则等待，并更新 router_busy_until。"""
        start_time = max(arrival_time, self.router_busy_until[pos])
        self.router_busy_until[pos] = start_time + self.RouterCycle
        # print("self.inputFlitNum",self.inputFlitNum,"arrival_time",arrival_time,"self.router_busy_until[pos]",self.router_busy_until[pos],"start_time",start_time,"self.RouterCycle",self.RouterCycle,"pos", pos, "node", node)
        return start_time + self.RouterCycle
    ...
    def simple_forward(self, curLayerId, spineTokenId, timestep, flitNumber):
        curSpineInputTime = 0 # 考虑过数据依赖以及NoC的输入时间
    ...
    def forward(self, curLayerId, spineTokenId, timestep, flitNumber): # 通过NoC确定token真正的输入时间，考虑数据依赖以及NoC上的传输时间
        ...
        lastLayersId = self.connection[curLayerId] # 先寻找连接到该层的前面网络层
        self.inputFlitNum = self.inputFlitNum + flitNumber
        flitNumber = math.ceil(flitNumber*0.25)
        # if curLayerId.count("layer2.0.downsample.0") > 0: 
        #     print("curLayerId",curLayerId,"lastLayersId",lastLayersId)
        for lastLayerId in lastLayersId: # 查询每一层网络层
            ...
            if SNode.Id == TNode.Id: 
                # 查询输入时间，并考虑 router 占用：每个 flit 依次到达并占用，下一 spine 需等待
                first_arrival = self.tokenSpineOutputTime[lastLayerId][timestep][spineTokenId]
                pos = y1 * self.meshWidth + x1
                for flitId in range(flitNumber):
                    arrival = first_arrival + flitId * self.RouterCycle * SNode.occupy
                    curSpineInputTime = self._router_wait_and_forward(arrival, pos, SNode)
                ...
                if x1 == x2:
                    ...
                    for flitId in range(flitNumber): # 对于每一个输入的flit
                        ...
                        # curNocLinkTranTime = self._router_wait_and_forward(curNocLinkTranTime, y1 * self.meshWidth + x1, SNode)  # 源节点占用等待
                        # 这里分成六种情况讨论
                        while(1): # 一直走直到终点
                            if x_now == x2 and y_now == y2: # 到达终点
                                ...
                                if flitId < x_allocate_dx * flitNumber: # 往x轴正方向走
                                    ...
                                    curNocLinkTranTime = self._router_wait_and_forward(curNocLinkTranTime, pos1, curNode)
                                    curNocLinkTranTime = curNocLinkTranTime + math.ceil(self.tranOccupy[pos1][pos2] * self.NoCCycle)
                    ...
                    for flitId in range(flitNumber): # 对于每一个输入的flit
                        ...
                        while(1): # 一直走直到终点
                            if x_now == x2 and y_now == y2: # 到达终点
                                ...
                                # 归一化
                                # print("x_now", x_now, "y_now", y_now, SNode.Allocates,"y_allocate_dy",y_allocate_dy,"y_allocate_re_dy", y_allocate_re_dy, "x_allocate", x_allocate)
